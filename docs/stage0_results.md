...
| mod3 v4b s1 | 800 | −33.0 | −14.2 | 2640.8 | 差 14.2 ps(s2:−22.8、s1+c2:−20.6 均更差,系统性) |
...
- **0.8 ns 高频点的取舍**:v4a 收敛(PT +1.9,与 v3 +2.6 同水平);v4b 三种配置(s1/s2/s1+c2)最好 −14.2 ps,系统性差 ~15–20 ps——冗余树去掉的逐节点规范化恰好是重定时可利用的切分裕量。**工程结论:RTL 默认冗余树(松弛目标最优),0.8 ns 综合配置加 `+define+SAFEDOT_TREE_LEGACY`**(两形态同一 RTL、同一验证)。800 ps 点面积仍受重定时混计影响(见表注),面积结论以 1800 ps 点为准。
